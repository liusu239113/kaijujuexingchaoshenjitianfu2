...
import com.kaiju.awaken.game.RewardAds
...
internal fun GameView.drawPetScreen(c: Canvas) {
    ...
    r.text(c, "常驻池 · 单抽 " + Pets.COST_SINGLE + " 券 · 十连 " + Pets.COST_TEN + " 券", 34f, y + 52f, 11f, Palette.TEXT_DIM)
    r.text(
...
internal fun GameView.tapPet(id: String) {
    when {
        ...
        id.startsWith("pet_deploy_") -> {
            ...
            perm.petId = pid
            Save.savePerm(context, perm)
            run?.let { com.kaiju.awaken.game.RunService.recalcAll(it, perm) }
            audio.play("starup")
...
internal fun GameView.petPull(times: Int) {
    ...
    if (perm.petTickets < cost) {
        audio.play("error")
        showToast("宠缘券不足（当前 " + perm.petTickets + "），可用星尘兑换或看广告获取")
        return
    }
...
internal fun GameView.drawPetGachaOverlay(c: Canvas) {
    ...
    val btnTop = top + boxH - 218f
    button(c, "pet_roll1", "单 抽（" + Pets.COST_SINGLE + " 券）", 40f, btnTop, w - 80f, 48f, Palette.PINK)
    button(c, "pet_roll10", "十 连（" + Pets.COST_TEN + " 券）", 40f, btnTop + 56f, w - 80f, 48f, Palette.GOLD)
    ghostButton(c, "pet_ad", if (RewardAds.isReady()) "看广告得 1 券" else "看广告得券（接入中）", 40f, btnTop + 112f, w - 80f, 44f, Palette.CYAN)
    ghostButton(c, "pet_gacha_close", "关 闭", 40f, btnTop + 164f, w - 80f, 44f, Palette.TEXT_DIM)
}
