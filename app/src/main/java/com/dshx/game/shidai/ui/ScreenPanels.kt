...
import com.dshx.game.shidai.game.Equip
import com.dshx.game.shidai.game.Rarity
import com.dshx.game.shidai.game.RunService
import com.dshx.game.shidai.game.Save
import com.dshx.game.shidai.game.skillHits
import com.dshx.game.shidai.game.skillLvMul
    ...
    when (panel) {
        ...
        "skills" -> {
            ...
            for (s in hero.skills) {
                ...
                drawIcon(c, com.dshx.game.shidai.game.ArtIcon.skill(s), 68f, y + 44f, 46f, if (s.isUltimate) Palette.GOLD else Palette.CYAN)
        ...
        "items" -> {
            ...
            for (it in Content.allItems) {
                ...
                drawIcon(c, com.dshx.game.shidai.game.ArtIcon.item(it.id), 58f, y + 34f, 40f, Palette.GOLD)
...
private fun heroSkillLevel(p: com.dshx.game.shidai.game.RunState, skillId: String): Int =
...
internal fun GameView.tapPanel(id: String) {
    ...
    when (id) {
        ...
        "panel_reset_do" -> {
            ...
            perm = com.dshx.game.shidai.game.PermState()
            Save.savePerm(context, perm)
            run = null
            battle = null
            panel = ""
        ...
        "confirm_yes" -> {
            ...
            when (act) {
                ...
                "export" -> {
                    ...
                    // 看广告解锁职阶
                    if (act.startsWith("adclass_")) {
                        val cid = act.removePrefix("adclass_")
                        requestAd(com.dshx.game.shidai.game.RewardAds.PLACEMENT_CLASS_UNLOCK) {
                            perm.unlocked.add("class:" + cid)
                            Save.savePerm(context, perm)
                            setupClass = cid
                            showToast("已解锁职阶：" + (com.dshx.game.shidai.game.Data.classById[cid]?.name ?: ""))
                        }
                    }
                }
            }
...
private fun GameView.drawBattleEndOverlay(c: Canvas) {
    ...
    val canRevive = !win && run?.adReviveUsed != true
    val canDouble = win && !rewardDoubleUsed
    val footerH = when {
        canRevive -> 170f
        canDouble -> 136f
        else -> if (win) 74f else 106f
    }
    ...
    if (win) {
        if (canDouble) {
            // 高转化位：战果翻倍（每场一次）
            button(c, "over_double_reward", "看广告 · 战果翻倍", 48f, btnTop, w - 96f, 54f, Palette.GOLD)
            button(c, "over_continue", "继 续 深 入", 48f, btnTop + 62f, w - 96f, 50f, Palette.CYAN)
            r.text(c, "金币与经验再结算一次，本场仅一次机会", w / 2f, btnTop + 132f, 10.5f, Palette.TEXT_DIM, false, Paint.Align.CENTER)
        } else {
        ...
        if (canRevive) {
            // 高转化位：看广告原地复活（每局一次）
            button(c, "over_ad_revive", "看广告 · 原地复活", 48f, btnTop, w - 96f, 54f, Palette.GREEN)
            button(c, "over_reincarnate", "轮 回", 48f, btnTop + 62f, w - 96f, 50f, Palette.PINK)
            r.text(c, "复活后本轮继续；轮回则把进度折算为神格点。", w / 2f, btnTop + 132f, 10.5f, Palette.TEXT_DIM, false, Paint.Align.CENTER)
        } else {
...
internal fun GameView.tapOverlay(id: String) {
    when (id) {
        ...
        "over_ad_revive" -> requestAd(com.dshx.game.shidai.game.RewardAds.PLACEMENT_REVIVE) {
            val b = battle
            val p = run
            if (b != null && p != null) {
                p.adReviveUsed = true
                ...
                showToast("复活成功：全队以 50% 生命继续战斗")
            }
        }
        "over_double_reward" -> requestAd(com.dshx.game.shidai.game.RewardAds.PLACEMENT_DOUBLE_REWARD) {
            val rw = lastRewards
            val p = run
            if (rw != null && p != null && !rewardDoubleUsed) {
                rewardDoubleUsed = true
                p.gold += rw.gold
                p.exp += rw.exp
                Save.saveRun(context, p, perm)
                showToast("战果翻倍：金币 +" + rw.gold + "，经验 +" + rw.exp)
            }
        }
    }
}
