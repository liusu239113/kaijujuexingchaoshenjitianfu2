...
class Battle(
    ...
    /** 看广告复活：全队以 [pct] 生命站起来，战斗继续（每局限一次，由调用方记录）。 */
    fun reviveParty(pct: Double) {
        for (u in allies) {
            u.alive = true
            u.hp = u.stats.maxHp * pct
            u.shield = 0.0
            u.energy = u.stats.energyMax * 0.5
            u.buffs.removeAll { it.isDebuff }
        }
        // 敌人保留当前血线：复活不该直接白送一场胜利
        finished = false
        victory = false
        awaitingInput = false
        auto = false
        addLog("广告复活：全队以 " + (pct * 100).toInt() + "% 生命重新站起。")
    }
