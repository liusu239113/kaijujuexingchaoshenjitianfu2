...
internal fun GameView.drawHubScreen(c: Canvas) {
    ...
    if (hero != null) {
        ...
        val emblemKey = promos.lastOrNull()?.let { com.dshx.game.shidai.game.ArtIcon.emblem(it.id) }
    ...
    val gold = p?.gold ?: 0
    val cols = listOf(
        com.dshx.game.shidai.game.ArtIcon.GOLD to gold.toString(),
        com.dshx.game.shidai.game.ArtIcon.TALENT to perm.talentPoints.toString(),
        com.dshx.game.shidai.game.ArtIcon.DUST to perm.dust.toString(),
        com.dshx.game.shidai.game.ArtIcon.FLOOR to perm.bestFloor.toString()
    )
    val cw = (w - 40f) / 4f
...
internal fun GameView.tapHub(id: String) {
    ...
    when (id) {
        ...
        "hub_recruit" -> {
            recruitList = ArrayList(com.dshx.game.shidai.game.TowerService.tavernCandidates(run?.floor ?: 1))
