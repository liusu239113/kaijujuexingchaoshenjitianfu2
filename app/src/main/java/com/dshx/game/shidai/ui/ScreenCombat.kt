...
import com.dshx.game.shidai.game.TargetKind
...
private fun GameView.drawUnitRow(c: Canvas, list: List<Unit>, top: Float, rowH: Float, isEnemy: Boolean, b: Battle, accent: Int) {
    ...
    for (i in list.indices) {
        ...
        val cpIcon = if (isEnemy) "" else com.dshx.game.shidai.game.ArtIcon.companion(key)
...
private fun GameView.drawCommandBar(c: Canvas, b: Battle) {
    ...
    for (i in usable.indices) {
        ...
        drawIcon(c, com.dshx.game.shidai.game.ArtIcon.skill(s), x + 20f, y + 24f, 30f, accent)
    ...
    if (itemIds.isNotEmpty()) {
        ...
        for (id in itemIds.take(5)) {
            val def = com.dshx.game.shidai.game.Content.itemById[id] ?: continue
            val cnt = p.items[id] ?: 0
            card(c, ix, iy - 10f, 62f, 44f, r.withAlpha(Palette.GOLD, 190), 10f)
            drawIcon(c, com.dshx.game.shidai.game.ArtIcon.item(id), ix + 20f, iy + 12f, 30f, Palette.GOLD)
...
internal fun GameView.tapCombat(id: String) {
    ...
    when {
        ...
        id.startsWith("cb_item_") -> {
            ...
            b.useItem(iid)
            audio.play(com.dshx.game.shidai.game.ArtIcon.itemSfx(iid))
            selectedSkill = null
        }
